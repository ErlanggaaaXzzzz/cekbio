...
const initWhatsApp = async (sessionName, userId) => {
  ...
  waClients[sessionName] = { sock, status: 'connecting', qr: null };
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      if (shouldReconnect) {
        initWhatsApp(sessionName, userId);
      } else {
...
app.post('/api/whatsapp/connect', authenticateToken, async (req, res) => {
  try {
    const client = await initWhatsApp(req.user.sessionName, req.user.id);
    if(client.qr && client.status === 'disconnected') {
